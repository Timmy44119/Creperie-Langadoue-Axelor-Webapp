...
public class InvoiceController {
	...
	/**
	 * Called from invoice form view on partner change. Get the default price list
	 * for the invoice. Call
	...
	public void fillPriceList(final ActionRequest request, final ActionResponse response) {
		try {
			...
			final int priceListTypeSelect = Beans.get(InvoiceService.class).getPurchaseTypeOrSaleType(invoice);

			if (invoice.getPartner() != null) {
				// Select the price list
				final PriceList priceList = Beans.get(PartnerPriceListService.class).getDefaultPriceList(partner,
						priceListTypeSelect);
