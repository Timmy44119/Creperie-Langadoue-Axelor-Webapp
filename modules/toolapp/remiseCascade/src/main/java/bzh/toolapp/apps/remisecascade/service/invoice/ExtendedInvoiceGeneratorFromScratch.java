...
public class ExtendedInvoiceGeneratorFromScratch extends InvoiceGenerator {
  ...
  @Inject protected TaxInvoiceLine taxInvoiceLine;

  public ExtendedInvoiceGeneratorFromScratch(
      final int operationType,
      final Company company,
      final PaymentCondition paymentCondition,
      final PaymentMode paymentMode,
      final Address mainInvoicingAddress,
      final Partner partner,
      final Partner contactPartner,
  ...
  public void computeInvoice(final Invoice invoice) throws AxelorException {
    ...
    for (final InvoiceLine invoiceLine : invoiceLineList) {
      ...
      invoice.setExTaxTotal(
          invoice
              .getExTaxTotal()
              .add(invoiceLine.getExTaxTotal().setScale(2, BigDecimal.ROUND_HALF_UP)));
  ...
  private List<InvoiceLine> computeValueLines(
      final Invoice invoice, final List<InvoiceLine> invoiceLineList) throws AxelorException {
  ...
  // Mise � jour des montant de chaque ligne de la facture
  private InvoiceLine updateInvoiceLine(final Invoice invoice, final InvoiceLine invoiceLine)
      throws AxelorException {
    ...
    if (!invoice.getInAti()) {
      ...
      exTaxTotal =
          this.computeGlobalDiscount(invoice, exTaxTotal).setScale(2, BigDecimal.ROUND_HALF_UP);
  ...
  protected BigDecimal computeGlobalDiscount(final Invoice invoice, BigDecimal totalAmount) {
    ...
    if (invoice.getDiscountTypeSelect() != PriceListLineRepository.AMOUNT_TYPE_NONE) {
      totalAmount =
          priceListService.computeDiscount(
              totalAmount, invoice.getDiscountTypeSelect(), invoice.getDiscountAmount());
    }
    ...
    if (invoice.getDiscountTypeSelect() != PriceListLineRepository.AMOUNT_TYPE_NONE) {
      totalAmount =
          priceListService.computeDiscount(
              totalAmount, invoice.getSecDiscountTypeSelect(), invoice.getSecDiscountAmount());
    }
  ...
  public BigDecimal computeDiscount(final InvoiceLine invoiceLine, final Boolean inAti) {
    ...
    final BigDecimal secondDiscount =
        this.priceListService.computeDiscount(
            firstDiscount,
            invoiceLine.getSecDiscountTypeSelect(),
            invoiceLine.getSecDiscountAmount());
