...
public class StockMoveInvoiceController {
  ...
  public void generateInvoice(final ActionRequest request, final ActionResponse response) {
    try {
      ...
      for (final Long id : partnerId) {
        ...
        final List<StockMove> stockMovesWoSo =
            this.getStockMoves(this.ORIGIN_TYPE_SELECT_NULL, id, stockMoves);
        ...
        final List<StockMove> stockMovesWSo =
            this.getStockMoves(StockMoveRepository.ORIGIN_SALE_ORDER, id, stockMoves);
  ...
  private List<StockMove> getStockMoves(
      final String typeStockMove, final Long idPartner, final List<StockMove> stockMovesList) {
  ...
  public void sendToInvoice(
      ...
      final List<StockMove> stockMoveList,
      final List<Long> stockMoveIdList)
      throws AxelorException {
    ...
    final boolean paymentConditionToCheck =
        (Boolean) mapResult.getOrDefault("paymentConditionToCheck", false);
    final boolean paymentModeToCheck =
        (Boolean) mapResult.getOrDefault("paymentModeToCheck", false);
    final boolean contactPartnerToCheck =
    ...
    if (paymentConditionToCheck || paymentModeToCheck || contactPartnerToCheck) {
      final ActionViewBuilder confirmView =
