...
public class SaleOrderLineController {
	...
	/**
	 * Overrides method "compute" from SaleOrderLineController in project
	 * axelor-sale.
	 *
	 * @param request
	 * @param response
	...
	private void compute(final ActionResponse response, final SaleOrder saleOrder, final SaleOrderLine orderLine)
			throws AxelorException {

		final Map<String, BigDecimal> map = Beans.get(SaleOrderLineService.class).computeValues(saleOrder, orderLine);
		...
		response.setAttr("priceDiscounted", "hidden", map.getOrDefault("priceDiscounted", BigDecimal.ZERO)
				.compareTo(saleOrder.getInAti() ? orderLine.getInTaxPrice() : orderLine.getPrice()) == 0);
	}
	...
	public void getDiscount(final ActionRequest request, final ActionResponse response) {
		...
		try {
			...
			if (saleOrderLine.getProduct().getInAti()) {
				discounts = saleOrderLineService.getDiscountsFromPriceLists(saleOrder, saleOrderLine,
						saleOrderLineService.getInTaxUnitPrice(saleOrder, saleOrderLine, saleOrderLine.getTaxLine()));
			} else {
				discounts = saleOrderLineService.getDiscountsFromPriceLists(saleOrder, saleOrderLine,
						saleOrderLineService.getExTaxUnitPrice(saleOrder, saleOrderLine, saleOrderLine.getTaxLine()));
			}

			if (discounts != null) {
				final BigDecimal price = (BigDecimal) discounts.get("price");
				if ((price != null)
						&& (price.compareTo(saleOrderLine.getProduct().getInAti() ? saleOrderLine.getInTaxPrice()
								: saleOrderLine.getPrice()) != 0)) {
					if (saleOrderLine.getProduct().getInAti()) {
						response.setValue("inTaxPrice", price);
						response.setValue("price",
								saleOrderLineService.convertUnitPrice(true, saleOrderLine.getTaxLine(), price));
					} else {
						response.setValue("price", price);
						response.setValue("inTaxPrice",
				...
				if ((saleOrderLine.getProduct().getInAti() != saleOrder.getInAti()) && ((Integer) discounts.get(
					...
					response.setValue("discountAmount",
							saleOrderLineService.convertUnitPrice(saleOrderLine.getProduct().getInAti(),
				...
				this.logger.debug("seconde remise, type de sélection = {}, montant = {}",
				...
				if ((saleOrderLine.getProduct().getInAti() != saleOrder.getInAti()) && ((Integer) discounts.get(
						PriceListConstants.LINE_SECOND_DISCOUNT_TYPE_SELECT) != PriceListLineRepository.AMOUNT_TYPE_PERCENT)) {
					response.setValue("secDiscountAmount",
							saleOrderLineService.convertUnitPrice(saleOrderLine.getProduct().getInAti(),
									saleOrderLine.getTaxLine(),
				...
				response.setValue("secDiscountTypeSelect",
						discounts.get(PriceListConstants.LINE_SECOND_DISCOUNT_TYPE_SELECT));
			}
