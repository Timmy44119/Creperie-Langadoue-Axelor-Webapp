...
public class ExtendedInvoiceGeneratorFromScratch extends InvoiceGenerator {
  ...
  public ExtendedInvoiceGeneratorFromScratch(
      final Invoice invoiceParam, final PriceListService priceListServiceParam)
  ...
  public void computeInvoice(final Invoice invoice) throws AxelorException {
    ...
    for (final InvoiceLine invoiceLine : invoice.getInvoiceLineList()) {
      ...
      final BigDecimal intermediateExTaxPrice =
          this.computeGlobalDiscountPerLine(lineExTaxTotal, invoice)
  ...
  private BigDecimal computeGlobalDiscountPerLine(
      final BigDecimal originalPrice, final Invoice invoice) {
    /*
    ...
    final BigDecimal firstDiscount =
        this.priceListService.computeDiscount(
            originalPrice, invoice.getDiscountTypeSelect(), invoice.getDiscountAmount());
    // then second discount
    final BigDecimal secondDiscount =
        this.priceListService.computeDiscount(
            firstDiscount, invoice.getSecDiscountTypeSelect(), invoice.getSecDiscountAmount());
