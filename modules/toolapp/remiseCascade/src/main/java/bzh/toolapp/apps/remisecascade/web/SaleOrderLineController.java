	...
	public void getDiscount(final ActionRequest request, final ActionResponse response) {
		...
		try {
			...
			if (discounts != null) {
				...
				if ((saleOrderLine.getProduct().getInAti() != saleOrder.getInAti()) && ((Integer) discounts
						.get("discountTypeSelect") != PriceListLineRepository.AMOUNT_TYPE_PERCENT)) {
					response.setValue("discountAmount",
							saleOrderLineService.convertUnitPrice(saleOrderLine.getProduct().getInAti(),
									saleOrderLine.getTaxLine(), (BigDecimal) discounts.get("discountAmount")));
				} else {
				...
				this.logger.debug("seconde remise, type de sélection = {}, montant = {}",
						discounts.get("lineSecondDiscountTypeSelect"), discounts.get("lineSecondDiscountAmount"));
				if ((saleOrderLine.getProduct().getInAti() != saleOrder.getInAti()) && ((Integer) discounts
						.get("lineSecondDiscountTypeSelect") != PriceListLineRepository.AMOUNT_TYPE_PERCENT)) {
